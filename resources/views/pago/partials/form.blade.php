		...
		<div class="col-md-4 col-sm-12"> 
			<select style="visibility: display;" id="doctorAsignado" class="form-control" name="doctorAsignado">
				...
				@foreach($users as $doctor)
					<option value="{{$doctor->nombre1.' '.$doctor->nombre2.' '.$doctor->nombre3.' '.$doctor->apellido1.' '.$doctor->apellido2}}">{{$doctor->nombre1.' '.$doctor->nombre2.' '.$doctor->nombre3.' '.$doctor->apellido1.' '.$doctor->apellido2.'- '.$doctor->numeroJunta}}</option>
				@endforeach
			</select>
		</div>
</div>
